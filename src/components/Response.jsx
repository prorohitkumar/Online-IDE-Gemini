...
import { Tabs, Tab, Box, IconButton, List, ListItem, ListItemText, Collapse, Typography, Paper } from '@mui/material';
...
const IssueList = ({ issues, title }) => {
  ...
  return (
    <Paper style={{ marginBottom: '10px' }}>
      <ListItem button onClick={handleClick} style={{ backgroundColor: '#A052B0', color: 'white' }}>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          {issues.map((issue, index) => (
            <ListItem key={index} style={{ paddingLeft: '20px', backgroundColor: getColorByCriticality(issue.criticalityLevel) }}>
              <ListItemText
...
const calculateIssueCounts = (reviewData) => {
  ...
  allIssues.forEach(issue => {
    ...
    else if (issue.criticalityLevel === 2 || issue.criticalityLevel === 3) counts.moderate += 1;
...
const IssueSummary = ({ counts }) => (
  <Box display="flex" justifyContent="space-between" alignItems="center" padding="10px" bgcolor="#1e1e1e">
    <Box>
      <Typography variant="h6" style={{ color: '#FF6347' }}>{`${counts.critical} Critical issues`}</Typography>
      <Typography variant="h6" style={{ color: '#FFD700' }}>{`${counts.moderate} Moderate issues`}</Typography>
      <Typography variant="h6" style={{ color: '#32CD32' }}>{`${counts.normal} Normal issues`}</Typography>
    </Box>
...
function Response({ code, setCode, fixedCode, enhancedCode, errorDescription, errorMessage }) {
  ...
  const [issueCounts, setIssueCounts] = useState({ critical: 0, moderate: 0, normal: 0 });
  ...
  return (
    <div style={{ backgroundColor: '#1e1e1e', color: 'white', padding: '10px', borderRadius: '5px', height: '99%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Tabs value={tabIndex} onChange={handleTabChange} textColor="inherit" className="custom-tabs">
      ...
      {showIssueSummary && <IssueSummary counts={issueCounts} />} {/* Show IssueSummary only if showIssueSummary is true */}
