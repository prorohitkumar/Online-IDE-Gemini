...
function CodeEditor({ file, onSave, setCode, setFixedCode, setEnhancedCode, setErrorDescription, setErrorMessage }) {
  const [content, setContent] = useState('');
  const [loading, setLoading] = useState(false); // Add loading state

  useEffect(() => {
    if (file) {
      file.handle.getFile().then(f => f.text()).then(setContent);
    } else {
  ...
  const isValidCode = (code) => {
    ...
    const validFileExtensions = ['.xml', '.yaml', '.yml', '.json', 'dockerfile', '.sql', '.sh', '.mk', '.gitignore', '.pom'];
    ...
    return !simpleEnglishTextRegex.test(code) || code.includes(';') || code.includes('{') || code.includes('}');
  };

  const handleReview = async () => {
    if (!isValidCode(content)) {
      setErrorMessage('Unfortunately, you have not provided any code for me to review. Please provide the code so that I can conduct the analysis and return the observations in the requested JSON format.');
      return;
    }
    ...
    try {
      ...
      if (review.error) {
        ...
        setErrorDescription(JSON.stringify({
          ...
          performanceOptimizationIssues: review.performanceOptimizationIssues,
      ...
      setErrorDescription(JSON.stringify({
        ...
        raw_response: reviewResponse ? await reviewResponse.text() : 'No response',
      }, null, 2));
    } finally {
  ...
  return (
    <div style={{ backgroundColor: '#1e1e1e', color: 'white', padding: '10px', borderRadius: '5px', height: '99%', position: 'relative' }}>
      {loading && <Loading />} {/* Show spinner if loading */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <Typography variant="h6" style={{ color: 'white' }}>
          {file ? file.name : 'No file selected'}
        </Typography>
        <Button variant="contained" color="primary" onClick={handleSave} style={{ height: '46px' }}>
          Save
        </Button>
      </div>
