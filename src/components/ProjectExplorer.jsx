...
function ProjectExplorer({ folder, onFileSelect }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', backgroundColor: '#000', color: '#fff' }}>
      {folder ? (
        <List>
          {folder.subFolders.map((subFolder) => (
            <div key={subFolder.path}>
              <ListItem
                ...
                style={activeFolder === subFolder.path ? activeFolderStyle : listItemStyle}
              >
                <ListItemIcon>
                  <FolderIcon style={{ color: '#f1c40f' }} />
                </ListItemIcon>
                <ListItemText primary={subFolder.name} />
                {open[subFolder.path] ? <ExpandLess style={{ color: '#fff' }} /> : <ExpandMore style={{ color: '#fff' }} />}
              </ListItem>
              <Collapse in={open[subFolder.path]} timeout="auto" unmountOnExit>
                <ProjectExplorer folder={subFolder} onFileSelect={onFileSelect} />
              </Collapse>
            </div>
          ))}
          {folder.files.map((file) => (
            <ListItem
              ...
              style={file.path === selectedFile ? selectedFileStyle : listItemStyle}
              onMouseOver={(e) => {
                e.currentTarget.style.backgroundColor = '#444';
              }}
              onMouseOut={(e) => {
                e.currentTarget.style.backgroundColor = file.path === selectedFile ? '#555' : '#000';
              }}
            >
              <ListItemIcon>
